...
export default function BlogListWithFilter({ allBlogs, allTags }: BlogListProps) {
          ...
          {allTags.map((tag) => {
            ...
            return (
              <button
                ...
                className={`inline-flex items-center gap-1.5 rounded-md border px-3 py-1.5 font-mono text-xs transition-all duration-200 cursor-pointer ${isSelected
                  ? "border-red-500 bg-red-500/10 text-red-500 font-medium dark:border-red-600 dark:bg-red-600/10 dark:text-red-400"
                  : "border-border/60 bg-secondary/50 text-muted-foreground hover:border-border hover:bg-secondary"
                  }`}
              >
                <div className={`flex h-3.5 w-3.5 items-center justify-center rounded border transition-all ${isSelected ? "border-red-500 bg-red-500 text-white dark:border-red-600 dark:bg-red-600" : "border-muted-foreground/40"
                  }`}>
        ...
        {filteredBlogs.length === 0 ? (
          <div className="py-16 text-center">
            <p className="text-sm italic text-muted-foreground lg:text-base">
              Belum ada artikel yang cocok dengan kombinasi filter tersebut.
            </p>
          </div>
        ) : (
          filteredBlogs.map((blog) => {
            const formattedDate = new Date(blog.datePublish).toLocaleDateString(
              "id-ID", { year: "numeric", month: "short", day: "numeric" }
            );

            return (
              <article key={blog.slug} className="group relative flex flex-col gap-4 py-9 transition-all duration-300 first:pt-0 last:pb-0 md:flex-row md:items-start md:gap-8 lg:gap-12 lg:py-11">
                ...
                <div className="flex shrink-0 items-center gap-3 font-mono text-xs text-muted-foreground md:w-36 md:flex-col md:items-start md:gap-1.5 lg:w-44 lg:text-sm">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="max-w-[120px] truncate lg:max-w-[140px]">{blog.author}</span>
                  </div>
                </div>
                ...
                <div className="flex-1 space-y-2.5 lg:max-w-2xl lg:space-y-3">
                  ...
                  <p className="text-sm leading-relaxed text-muted-foreground lg:text-base">{blog.description}</p>
                  <div className="relative z-10 flex flex-wrap gap-2 pt-2 lg:pt-3">
                    {blog.tags.map((tag) => (
                      <span key={tag} className="inline-flex items-center rounded-md border border-border/40 bg-secondary px-2 py-0.5 font-mono text-[10px] text-secondary-foreground lg:px-2.5 lg:py-1 lg:text-xs">
                        #{tag}
                      </span>
                    ))}
                  </div>
